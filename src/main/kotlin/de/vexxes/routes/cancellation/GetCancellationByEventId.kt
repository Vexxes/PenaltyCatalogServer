...
fun Route.getCancellationByEventId(
    app: Application,
    repository: CancellationRepository,
    validateBearerToken: ValidateBearerToken
) {
    get(Endpoint.GetCancellationByEventId.path) {
        if (validateBearerToken.validateAll(call.request.headers["Authorization"].toString())) {
            try {
                val id = call.parameters["eventId"].toString()
                val foundCancellation = repository.getCancellationForEvent(id).map(Cancellation::toDto)
                call.respond(
